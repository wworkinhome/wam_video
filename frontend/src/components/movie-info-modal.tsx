...
// navegar), con detalle completo y "Más títulos similares" al abrirse.
export function MovieInfoModal({ movie, activeProfileId }: { movie: Movie; activeProfileId?: string | null }) {
    ...
    if (next && !detail) {
      ...
      try {
        const [full, list] = await Promise.all([
          clientFetch<Movie>(`/movies/${movie.id}`),
          clientFetch<Paginated<Movie>>(`/movies?limit=13`),
        ]);
        setDetail(full);
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 flex flex-col gap-3 p-5 sm:p-6">
              ...
              <div className="flex flex-wrap gap-3">
                <Link href={`/ver/pelicula/${shown.id}`}>
                  <Button className="gap-2">
                    <Play className="size-4 fill-current" />
                    Reproducir
                  </Button>
                </Link>
                {activeProfileId && <FavoriteButton profileId={activeProfileId} movieId={shown.id} />}
              </div>
            </div>
          </div>
...
function rankSimilar(all: Movie[], current: Movie): Movie[] {
